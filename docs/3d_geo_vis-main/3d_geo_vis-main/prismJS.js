document.addEventListener("DOMContentLoaded", function() {
  require([
    ...
    const euPrismLayer = new FeatureLayer({
      url: "https://services1.arcgis.com/AGrMjSBR7fxJYLfU/arcgis/rest/services/EU_prism/FeatureServer/0",
      outFields: ["*"],
      visible: true,
      title: "EU Prism Map",
    ...
    function applyAll() {
      if (booting) return;
      euPrismLayer.renderer = buildRenderer(euPrismLayer.renderer);
      const op = Number(transparencyInput?.value);
      euPrismLayer.opacity = Number.isFinite(op) ? op : 1;
      const offset = Number(heightAboveGroundInput?.value);
      euPrismLayer.elevationInfo = { mode: "relative-to-ground", offset: Number.isFinite(offset) ? offset : 0 };
    }
